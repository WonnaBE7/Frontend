<template>
    <Card class="bg-white border border-gray-150">
      <Typography type="B_14_140" class="flex flex-row w-full items-center border-b border-gray-200 pb-2">
        <MessageCircle class="w-4 text-sub-aqua-p mr-2"/> 댓글 {{ comments.length }}
      </Typography>
      <div v-for="comment in comments" :key="comment.commentId" class="mt-4 w-full">
        <div class="flex flex-row  items-center gap-4">
          <Typography type="B_12_140">{{ comment.userName }} </Typography>
          <NoBorderTag color="bg-gray-150">{{ comment.nowme }}</NoBorderTag>
        </div>
        <Typography type="M_12_140" class="mt-1 text-gray-500">{{ comment.content }}</Typography>
        <div
          class="flex items-center gap-1 cursor-pointer mt-2"
          @click="toggleLike"
        >
          <HeartIcon
            :class="[
