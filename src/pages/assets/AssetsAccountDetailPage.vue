<template>
    <AppLayout>
        <Card v-if="detail" class="bg-sub-yellow-bg mb-4 sm:mb-6 md:mb-8 border border-sub-yellow-c">
        ...
        <div v-if="detail?.transactions?.length" v-for="(tx, idx) in detail.transactions" :key="idx" class="mb-4 sm:mb-6 md:mb-8">
            <Card class="border border-gray-150">
                <div class="flex flex-row w-full justify-between items-center">
                    ...
                    <Typography type="B_16_120">
                        {{ formatAmount(tx.amount).toLocaleString() }}원
                    </Typography>
                </div>
            </Card>
        </div>
  ...
  onMounted(async () => {
    detail.value = await getAccountDetail(props.accountId)
  })
  </script>