...
import { mockConsumptionSummaryMeta, mockEstimatedAndTodayConsumption, mockMonthlyCategoryConsumption, mockMonthlyConsumptionSummary, mockMonthlyTransactionDetail, mockTodayCategoryConsumption, mockTodayTransactionDetail, mockMonthlyCategoryDetailMap, mockTodayCategoryDetailMap } from "./consumption.mock"
...
export const getConsumption = async () => {
    if (USE_MOCK) {
        console.log('MOCK 모드: 소비 데이터 mock 사용')
    ...
    try {
        ...
        console.log('API 실패: 소비 데이터 mock 사용')
        return mockConsumptionSummaryMeta
    }
}
...
export const getEstimatedCategoryConsumption = async (yearMonth:string) => {
    try {
        ...
        return {
            categories: mockMonthlyCategoryConsumption.categories.map(c => ({
                consumptionCategory: c.consumptionCategory,
                amount: c.amount * 1.2,
...
export const getMonthlyCategoryDetail = async (consumptionDetailCategory: string, yearMonth : string) => {
    try {
        ...
        return mockMonthlyCategoryDetailMap[consumptionDetailCategory as keyof typeof mockMonthlyCategoryDetailMap] || mockMonthlyCategoryDetailMap.food
    }
}
...
export const getTodayCategoryDetail = async (consumptionDetailCategory: string) => {
    try {
        ...
        return mockTodayCategoryDetailMap[consumptionDetailCategory as keyof typeof mockTodayCategoryDetailMap] || mockTodayCategoryDetailMap.food
    }
}